...
class DBP15KRawNeighbors():
    ...
    def id_neighbors_loader(self):
        ...
        #     key = key + 1
        #     if key not in self.ent_reinfo.keys():
        #         self.ent_reinfo[key] = 0
        #ent_mean = data.shape[0] / (len(self.id_entity)*2)
        #ent_sum = sum( self.ent_reinfo)
        #self.ent_reinfo = dict(sorted(self.ent_reinfo.items()))  # sorted by key
        #self.ent_reinfo = [self.ent_reinfo[key] for key in self.ent_reinfo]
        # min = min(self.ent_reinfo)
        # max = max(self.ent_reinfo)
        # norm = max - min
        # mean = np.mean(self.ent_reinfo)
        # std = np.std(self.ent_reinfo)
        #self.ent_reinfo = [((self.ent_reinfo[key]-mean)/std) for key in self.ent_reinfo]
        #self.ent_reinfo = standardize(self.ent_reinfo)
        #self.ent_reinfo = normalize(self.ent_reinfo)
        ...
        # self.ent_reinfo = softmax(self.ent_reinfo)
